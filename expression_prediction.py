...
def showSteps():
   ...
   epilog += "-) Step5 analyze TSS profile in housekeeping vs. unexpressed genes and plot in R\n"
   epilog += "-) Step6 extract coverage parameters for expression prediction\n"
   epilog += "-) Step7 expression prediction\n"
   return epilog
...
def step3(ERR_LOG,OUT_LOG,args,proj_dir,script_dir,filenames):
    ...
    if not args.keep:
        subprocess.call(["rm",proj_dir+"/"+filenames["trimmed_fastq"]])
